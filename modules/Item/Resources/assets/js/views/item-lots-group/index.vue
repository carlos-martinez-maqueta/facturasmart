                                            ...
                                            <td>
                                                ...
                                                >
                                                    <el-option
                                                        ...
                                                        :key="idx"
                                                        :value="dt.id"
                                                        :label="dt.description.replace('Almacén -', '')"
                                                    ></el-option>
                                                </el-select>
                                            </td>
                                            <td>
...
export default {
    ...
    methods: {
        async changeWarehouse(row) {
            console.log(row);
            try {
                this.loadingState = true;
                const response = await this.$http(
                    `${this.resource}/update_warehouse?lot_id=${row.id}&warehouse_id=${row.warehouse_id}`
                );
                console.log(response);
                if (response.status == 200) {
                    this.$message.success("Almacén actualizado cambiado");
                }
            } catch (e) {
                console.log(e);
            } finally {
        ...
        async changeState(row) {
            ...
            try {
                this.loadingState = true;
                const response = await this.$http(
                    `${this.resource}/update_state?lot_id=${row.id}&state_id=${row.state_id}`
                );
                if (response.status == 200) {
                    this.$message.success("Estado cambiado");
                }
            } catch (e) {
                console.log(e);
            } finally {
