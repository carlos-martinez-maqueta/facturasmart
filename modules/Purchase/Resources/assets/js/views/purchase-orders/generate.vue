...
export default {
    ...
    props: ["purchase_quotation", "isQuotation"],
    data() {
        return {
    ...
    async created() {
        if (this.purchase_quotation && !this.isQuotation) {
            let { suppliers } = this.purchase_quotation;
            let [supplier] = Object.values(suppliers);

            if (supplier) {
                this.reloadDataSuppliers(supplier.supplier_id);
            }
        }
        await this.initForm();
    ...
    methods: {
        ...
        setData() {
            ...
            if (!this.isQuotation) {
                ...
                this.form.quotation_id = oc.id;
            }
            this.form.type_purchase_order = "goods";
            let {
                customer: { internal_id },
            } = oc;
            this.form.client_internal_id = internal_id;
            oc.items.forEach((it) => {
                it.unit_price = 0;
                it.total_igv = 0;
                it.total_value = 0;
        ...
        async inputUnitPrice(index) {
            this.form.items[index].item = await _.find(this.items, {
                id: this.form.items[index].item_id,
            });
            // this.form.unit_price = this.form.item.purchase_unit_price
            this.form.items[index].affectation_igv_type_id =
                this.form.items[index].item.purchase_affectation_igv_type_id;
            // this.form.item_unit_types = _.find(this.items, {'id': this.form.item_id}).item_unit_types
            await this.clickAddItem(index);
        },
        async clickAddItem(index) {
            this.form.items[index].item.unit_price =
                this.form.items[index].unit_price;
            this.form.items[index].item.presentation = [];
            this.form.items[index].affectation_igv_type = _.find(
                this.affectation_igv_types,
        ...
        async changeDateOfIssue() {
            ...
            console.log(
                "🚀 ~ changeDateOfIssue ~ this.form.date_of_issue:",
        ...
        changeCurrencyType() {
            ...
            if (this.form.items) {
                this.form.items.forEach((row) => {
                    ...
                    console.log(
                        "🚀 ~ file: generate.vue:836 ~ this.form.items.forEach ~ new_item:",
                        new_item
                    );
                    items.push(new_item);
                });
