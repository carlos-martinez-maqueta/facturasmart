...
if ($document_type_id == '80') {
    ...
    $observation = isset($data['additional_information']) ? $data['additional_information'][0] : '';
}
...
$payments = [];
if (get_class($document) == Document::class || get_class($document) == SaleNote::class) {
...
<tr>
    ...
    <td class="celda">{{ $data['zone_name']}}</td>
    <td class="celda">
        @if ($isSaleNote)
            {{ $document->seller_id == null ? $document->user->name : $document->seller->name }}
        @else
