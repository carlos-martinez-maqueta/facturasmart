...
></small>
            ...
            ></el-date-picker> -->
                ...
                >
                    <label for="dispatcher_id">Transportista</label>
                    <el-select
                        v-model="document.dispatcher_id"
                        filterable
                        clearable
                    >
                        <el-option
                ...
                <div class="col-lg-12" v-show="document.total > 0">
                    <div class="form-group pull-right">
                        <label class="control-label">
                            ...
                            {{ document.currency_type_id }}</label
                        >
                    </div>
                </div>
                <div
                ...
                <!-- propinas -->
                <template
                    v-if="configuration.enabled_tips_pos && isInvoiceDocument"
                >
                    <set-tip
                        class="full py-2 border-top mt-2"
...
export default {
    ...
    methods: {
        getDispatchers(){
            this.$http.get(`/dispatchers/records-all`).then((response) => {
                this.dispatchers = response.data.records;
            });
        },
        //showGenerate
        hasCashOpen() {
...
clickPrint(format) {
    ...
    .catch(error => {
                ...
                for (let i = 0; i < this.quantity; i++) {
                    ...
                    if (response.data.success) {
                        ...
                        this.progress = Math.floor(
                            (this.generated / this.quantity) * 100
                        );
                    }
                }
                ...
                new_items = items.map((row) => {
                    ...
                    if (row.item.IdLoteSelected) {
                        ...
                        if (Array.isArray(row.item.lots_group)) {
                            ...
                            let sale_lots_group = this.getSaleLotsGroup(
                                row.item.lots_group
                            );
        ...
        async create() {
            ...
            await this.$http
                ...
                .then((response) => {
                    ...
                    this.titleDialog =
                        `Pedido ${type}: ` + this.form.identifier;
                });
        ...
        changeDocumentType() {
            ...
            if (this.configuration.enabled_tips_pos && !this.isInvoiceDocument)
                this.cleanFormTip();
        },
