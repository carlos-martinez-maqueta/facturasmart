<template>
    <el-dialog
        :title="titleDialog"
        width="70%"
        :visible="showDialog"
        @open="create"
        :close-on-click-modal="false"
        :close-on-press-escape="false"
        append-to-body
        :show-close="false"
    >
        <div class="form-body">
            <div class="row">
                <template
                    v-if="
                        configuration.show_dispatcher_documents_sale_notes_order_note
                    "
                >
                    <div class="col-md-3 col-lg-3 col-12">
                        <label for="date">Vendedor</label>
                        <el-select
                            v-model="form.seller_id"
                            placeholder="Seleccionar"
                            style="width: 100%"
                            clearable
                            filterable
                            @change="getRecords(true)"
                        >
                            <el-option
                                v-for="seller in sellers"
                                :key="seller.id"
                                :value="seller.id"
                                :label="seller.name"
                            ></el-option>
                        </el-select>
                    </div>
                    <div class="col-md-3 col-lg-3 col-12">
                        <label for="date">Transportista</label>
                        <el-select
                            v-model="form.dispatcher_id"
                            placeholder="Seleccionar"
                            style="width: 100%"
                            clearable
                            filterable
                            @change="setDispatcherId"
                        >
                            <el-option
                                v-for="dispatcher in dispatchers"
                                :key="dispatcher.id"
                                :value="dispatcher.id"
                                :label="`${dispatcher.description}`"
                            ></el-option>
                        </el-select>
                    </div>
                    <div class="col-md-3 col-lg-3 col-12">
                        <label for="date">Fecha de emisión</label>
                        <el-date-picker
                            v-model="form.date_of_issue"
                            type="date"
                            placeholder="Fecha de emisión"
                            style="width: 100%"
                            value-format="yyyy-MM-dd"
                            @change="setDateOfIssue"
                        ></el-date-picker>
                    </div>
                </template>
                <div class="col-md-3 col-lg-3 col-12">
                    <label>Documento por defecto</label>
                    <el-select
                        v-model="configuration.default_order_document"
                        placeholder="Seleccionar"
                        @change="changeDefaultDocument"
                    >
                        <el-option
                            v-for="option in document_types"
                            :key="option.id"
                            :value="option.id"
                            :label="option.description"
                            :disabled="
                                soapCompany == '03' &&
                                (option.id == '01' || option.id == '03')
                            "
                        ></el-option>
                    </el-select>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12" v-loading="loading">
                    <div class="table-responsive mt-3">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th class="text-center">#</th>
                                    <th class="text-center">
                                        Seleccionar<br />
                                        Marcar todos
                                        <el-checkbox
                                            v-model="isAllSelected"
                                            @change="onSelectedAll"
                                        ></el-checkbox>
                                    </th>
                                    <th class="text-center">Fecha Emisión</th>
                                    <th>Cliente</th>
                                    <th>Pedido</th>
                                    <th class="text-end">Total</th>
                                    <th>Tipo comprobante</th>
                                    <th>Serie</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr
                                    v-for="(row, index) in records"
                                    :key="index"
                                >
                                    <td class="text-center">
                                        {{ customIndex(index) }}
                                    </td>
                                    <td class="text-center">
                                        <el-checkbox
                                            v-model="row.selected"
                                            @change="changeSelected(row, index)"
                                        ></el-checkbox>
                                    </td>
                                    <td class="text-center">
                                        {{ row.date_of_issue }}
                                    </td>
                                    <td>
                                        {{ row.customer_name }}<br /><small
                                            v-text="row.customer_number"
                                        ></small>
                                    </td>
                                    <td>{{ row.identifier }}</td>
                                    <td class="text-end">{{ row.total }}</td>
                                    <td>
                                        <el-select
                                            v-model="row.document_type_id"
                                            @change="
                                                changeDocumentType(row, index)
                                            "
                                        >
                                            <el-option
                                                v-for="option in row.document_types"
                                                :key="option.id"
                                                :value="option.id"
                                                :label="option.description"
                                            ></el-option>
                                        </el-select>
                                    </td>
                                    <td>
                                        <el-select v-model="row.series_id">
                                            <el-option
                                                v-for="option in row.series"
                                                :key="option.id"
                                                :value="option.id"
                                                :label="option.number"
                                            ></el-option>
                                        </el-select>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                        <div>
                            <el-pagination
                                @current-change="getRecords()"
                                layout="total, prev, pager, next"
                                :total="pagination.total"
                                :current-page.sync="pagination.current_page"
                                :page-size="pagination.per_page"
                            >
                            </el-pagination>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="form-actions text-end pt-2">
            <el-button @click.prevent="close()">Cerrar</el-button>
            <el-button
                type="primary"
                @click="submit"
                v-if="documents.length > 0"
                :loading="loading_submit"
                >Generar</el-button
            >
        </div>
    </el-dialog>
</template>

<script>
import moment from "moment";
import queryString from "query-string";

export default {
    props: ["showDialog", "configuration", "soapCompany"],
    data() {
        return {
            titleDialog: "Generar comprobantes",
            resource: "order-notes/documents",
            documents: [],
            search_series_by_barcode: false,
            loading: false,
            loading_submit: false,
            records: [],
            all_document_types: [
                {
                    id: "01",
                    description: "Factura electrónica",
                },
                {
                    id: "03",
                    description: "Boleta electrónica",
                },
                {
                    id: "80",
                    description: "Nota de Venta",
                },
            ],
            document_types: [],
            all_series: [],
            pagination: {},
            dispatchers: [],
            establishment: {},
            isAllSelected: false,
            sellers: [],
            form: {
                document_type_id: null,
                series_id: null,
                dispatcher_id: null,
            },
        };
    },
    async created() {
        await this.getTables();
    },
    methods: {
        setDispatcherId() {
            if (!this.form.dispatcher_id) {
                return;
            }
            this.documents.map((doc) => {
                doc.dispatcher_id = this.form.dispatcher_id;
            });
        },
        setDateOfIssue() {
            if (!this.form.date_of_issue) {
                return;
            }
            this.documents.map((doc) => {
                doc.date_of_issue = this.form.date_of_issue;
                doc.date_of_due = this.form.date_of_issue;
            });
        },
        async changeDefaultDocument() {
            try {
                this.loading = true;
                let configuration = {
                    ...this.configuration,
                    default_order_document:
                        this.configuration.default_order_document,
                };
                const response = await this.$http.post(
                    `/configurations`,
                    configuration
                );
                if (response.data.success) {
                    this.$message.success(response.data.message);
                    this.changeDocumentAll();
                } else {
                    this.$message.error(response.data.message);
                }
            } catch (error) {
                this.$message.error(error.response.data.message);
            } finally {
                this.loading = false;
            }
        },
        changeDocumentAll() {
            let { default_order_document } = this.configuration;
            if (this.soapCompany == "03") {
                default_order_document = "80";
            }
            this.records.map((row, i) => {
                if (default_order_document == "01") {
                    let {
                        order_note: {
                            customer: { identity_document_type_id },
                        },
                    } = row;
                    if (identity_document_type_id == "6") {
                        row.document_type_id = "01";
                    } else {
                        row.document_type_id = "03";
                    }
                } else {
                    row.document_type_id = default_order_document;
                }
                this.changeDocumentType(row, i);
            });
        },
        onSelectedAll(value) {
            if (value) {
                this.records.map((row, i) => {
                    row.selected = true;
                    this.addDocument(row, i);
                });
                this.isAllSelected = true;
            } else {
                this.records.map((row, i) => {
                    row.selected = false;
                });
                this.documents = [];
                this.isAllSelected = false;
            }
        },
        async getTables() {
            await this.$http
                .get(`/order-notes/document_tables`)
                .then((response) => {
                    // this.document_types = response.data.document_types_invoice;
                    this.all_series = response.data.series;
                    this.establishment = response.data.establishment;
                    this.sellers = response.data.sellers;
                    this.dispatchers = response.data.dispatchers;
                });
        },
        changeDocumentType(row, index) {
            this.filterSeries(row, index);
        },
        filterSeries(row, index) {
            this.records[index].series_id = null;
            this.records[index].series = _.filter(this.all_series, {
                establishment_id: this.establishment.id,
                document_type_id: this.records[index].document_type_id,
                contingency: 0,
            });
            this.records[index].series_id =
                this.records[index].series.length > 0
                    ? this.records[index].series[0].id
                    : null;
        },
        validateDocuments() {
            let error_by_item = 0;
            this.documents.forEach((item) => {
                if (!item.series_id) error_by_item++;
            });

            return {
                error_by_item: error_by_item,
            };
        },
        async submit() {
            let validate = await this.validateDocuments();
            if (
                this.configuration
                    .show_dispatcher_documents_sale_notes_order_note
            ) {
                if (!this.form.dispatcher_id) {
                    return this.$message.error(
                        "El campo transportista es obligatorio"
                    );
                }
                if (!this.form.date_of_issue) {
                    return this.$message.error(
                        "El campo fecha de emisión es obligatorio"
                    );
                }
            }
            if (validate.error_by_item > 0) {
                return this.$message.error("El campo serie es obligatorio");
            }

            this.loading_submit = true;

            this.$http
                .post(`/${this.resource}`, { documents: this.documents })
                .then((response) => {
                    if (response.data.success) {
                        this.$message.success(response.data.message);
                        this.$eventHub.$emit("reloadData");
                        this.close();
                    } else {
                        this.$message.error(response.data.message);
                    }
                })
                .catch((error) => {
                    if (error.response.status === 422) {
                        this.errors = error.response.data;
                    } else {
                        this.$message.error(error.response.data.message);
                    }
                })
                .then(() => {
                    this.loading_submit = false;
                });
        },
        async changeSelected(row, index) {
            if (row.selected) {
                await this.addDocument(row, index);
            } else {
                this.deleteDocument(row);
            }
        },
        async addDocument(row, index) {
            let doc = await this.getDocument(row, index);
            console.log("🚀 ~ addDocument ~ doc:", doc);
            this.documents.push(doc);
        },
        async getDocument(row, index) {
            let q = await this.records[index].order_note;

            row.establishment_id = q.establishment_id;
            // row.date_of_issue = q.date_of_issue
            row.time_of_issue = moment().format("HH:mm:ss");
            row.customer_id = q.customer_id;
            row.currency_type_id = q.currency_type_id;
            row.purchase_order = null;
            row.exchange_rate_sale = q.exchange_rate_sale;
            row.total_prepayment = q.total_prepayment;
            row.total_charge = q.total_charge;
            row.total_discount = q.total_discount;
            row.total_exportation = q.total_exportation;
            row.total_free = q.total_free;
            row.total_taxed = q.total_taxed;
            row.total_unaffected = q.total_unaffected;
            row.total_exonerated = q.total_exonerated;
            row.total_igv = q.total_igv;
            row.total_base_isc = q.total_base_isc;
            row.total_isc = q.total_isc;
            row.total_base_other_taxes = q.total_base_other_taxes;
            row.total_other_taxes = q.total_other_taxes;
            row.total_taxes = q.total_taxes;
            row.total_value = q.total_value;
            row.total = q.total;
            row.operation_type_id = "0101";
            row.date_of_due = q.date_of_issue;
            row.items = q.items;
            row.charges = q.charges;
            row.discounts = q.discounts;
            row.attributes = [];
            row.payments = [];
            row.guides = q.guides;
            row.additional_information = null;
            row.actions = {
                format_pdf: "a4",
            };
            row.hotel = {};
            row.number = "#";

            row.type_period = null;
            row.quantity_period = null;
            row.prefix = "NV";

            row.order_note_id = this.records[index].index_id;
            row.seller_id = q.user_id;
            if (
                this.configuration
                    .show_dispatcher_documents_sale_notes_order_note
            ) {
                row.date_of_issue = this.form.date_of_issue;
                row.date_of_due = this.form.date_of_issue;
                row.dispatcher_id = this.form.dispatcher_id;
            }
            return row;
        },
        deleteDocument(row) {
            _.remove(this.documents, { index_id: row.index_id });
        },
        customIndex(index) {
            return (
                this.pagination.per_page * (this.pagination.current_page - 1) +
                index +
                1
            );
        },
        getRecords(init_current_page = false) {
            if (init_current_page) {
                this.pagination.current_page = 1;
            }

            this.loading = true;

            return this.$http
                .get(`/${this.resource}?${this.getQueryParameters()}`)
                .then((response) => {
                    this.records = response.data.data;
                    // let { default_order_document } = this.configuration;

                    this.changeDocumentAll();
                    this.pagination = response.data.meta;
                    this.pagination.per_page = parseInt(
                        response.data.meta.per_page
                    );
                    this.checkedOrderNote();
                })
                .catch((error) => {})
                .then(() => {
                    this.loading = false;
                });
        },
        checkedOrderNote() {
            _.forEach(this.records, (row) =>
                _.set(row, "selected", this.verifyOrderNote(row))
            );
        },
        verifyOrderNote(row) {
            let record = _.find(this.documents, { index_id: row.index_id });

            return record ? true : false;
        },
        getQueryParameters() {
            return queryString.stringify({
                page: this.pagination.current_page,
                limit: this.limit,
                seller_id: this.form.seller_id,
            });
        },
        async create() {
            this.form = {
                seller_id: null,
                date_of_issue: moment().format("YYYY-MM-DD"),
                dispatcher_id: null,
            };
            if (this.soapCompany == "03") {
                this.configuration.default_order_document = "80";
                await this.changeDefaultDocument();
                this.document_types = _.filter(this.all_document_types, {
                    id: "80",
                });
            } else {
                this.document_types = this.all_document_types;
            }
            await this.initForm();
            await this.getRecords(true);
        },
        initForm() {
            this.documents = [];
        },
        close() {
            this.$emit("update:showDialog", false);
        },
    },
};
</script>
