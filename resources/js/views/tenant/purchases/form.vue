<template>
                ...
                <div class="form-body">
                    ...
                    <div class="row">
                        ...
                        <!-- <div class="col-lg-3">
                            <div class="form-group" :class="{'has-danger': errors.payment_method_type_id}">
                                ...
                                <el-select v-model="form.payment_method_type_id" filterable @change="changePaymentMethodType">
                                    <el-option v-for="(option,idx) in payment_method_types" :key="idx" :value="option.id" :label="option.description"></el-option>
                                </el-select>
                                <small class="text-danger" v-if="errors.payment_method_type_id" v-text="errors.payment_method_type_id[0]"></small>
                            </div>
                        </div> -->
                        <div class="col-lg-2">
                            <div
                        ...
                        <div
                            ...
                            v-if="
                                config.enabled_global_igv_to_purchase === true
                            "
                        >
                            <div class="form-group">
                                <el-checkbox
                    ...
                    <div
                        v-if="form.items !== undefined && form.items.length > 0"
                        class="row"
                    >
                        <div class="col-md-12">
                                ...
                                <table class="table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-end">Precio V. actual</th>
    ...
    <!-- <small>
        <el-checkbox
            v-model="row.include_igv"
            :disabled="row.include_igv"
        >
            Incluye IGV (18%)
        </el-checkbox>
    </small> -->
                ...
                <!-- {{ calculateFinalPrice(row) }} -->
                                            ...
                                            <td class="text-end">
                                                <button
                                                    v-if="
                                                        applyLotsGroup(row.item)
                                                    "
                                                    class="btn waves-effect waves-light btn-sm btn-info mr-2"
                                                    type="button"
                                                    @click.prevent="
                                                        clickOpenLotsGroup(
                                                            index
        ...
        <series-form ref="series_form" @addRowLot="addRowLot"> </series-form>
...
export default {
    props: ["purchase_order_id","configuration"],
    components: {
        ResponsibleModal,
        LicenseModal,
        PurchaseFormItem,
    ...
    async mounted() {
        ...
        await this.$http
            ...
            .then((response) => {
                ...
                console.log("🚀 ~ .then ~ configuration:", this.configuration)
    ...
    async created() {
        ...
        let { decimal_quantity } = this.config;
        if (decimal_quantity) {
    ...
    methods: {
        calculateFinalPrice(row, index) {
        let { valor_ganancia, unit_price, include_igv } = row;
        let valor_ganancia_number = Number(valor_ganancia);
        let finalPrice = unit_price * valor_ganancia_number;
        if (include_igv) {
            finalPrice *= 1.18;
        }
        row.item.sale_unit_price = finalPrice;
        this.$set(this.form.items, index, row); // Ensure reactive updates
        return finalPrice.toFixed(2); // Format the result to two decimals
    },
        ...
        async existPurchase() {
            ...
            try {
                this.loading = true;
                const response = await this.$http.post("/purchases/exist", {
        ...
        async openFormItem() {
            let exist = await this.existPurchase();
            if (exist) {
                this.$message({
                    type: "warning",
            ...
            if (this.purchase_order_id) {
                await this.$http
                    ...
                    .then((response) => {
                        ...
                        this.form.total_exportation =
                            purchase_order.total_exportation;
                        this.form.total_taxed = purchase_order.total_taxed;
                        this.form.total_exonerated =
                            purchase_order.total_exonerated;
                        this.form.total_unaffected =
                            purchase_order.total_unaffected;
        ...
        changePercentageIgv(row) {
            let igv = this.form.igv_10 ? 10 / 100 : 18 / 100;
            let { item, unit_value } = row;
            let { has_igv } = item;
            row.percentage_igv = igv * 100;
            if (has_igv) {
                row.unit_value = row.unit_price / (igv + 1);
            } else {
                row.unit_price = unit_value * (igv + 1);
            }

            row.total_igv_without_rounding =
                row.unit_value * igv * row.quantity;
            row.total_igv = _.round(row.total_igv_without_rounding, 2);
            row.total_taxes = _.round(row.total_igv_without_rounding, 2);
            row.total_taxes_without_rounding = row.total_igv_without_rounding;
            row.purchase_unit_price = (row.unit_value * (igv + 1)).toFixed(2);
            row.total = _.round(row.unit_price * row.quantity, 2);
            row.total_value = _.round(row.unit_value * row.quantity, 2);
            row.total_value_without_rounding = row.unit_value * row.quantity;
            row.total_without_rounding = row.unit_price * row.quantity;
            return row;
        },
        ...
        addRow(row) {
            ...
            if (this.currentItem && this.currentIndex != null) {
                this.form.items[this.currentIndex] = row;
                this.calculateFinalPrice(row,this.currentIndex); 
        


            } else {
                this.form.items.push(row);
                let lastIndex = this.form.items.length -1;
                this.calculateFinalPrice(row,lastIndex);             
            }
            this.calculateTotal();
        },
        clickRemoveItem(index) {
            ...
            this.form.items.forEach((row) => {
                ...
                if (this.isFromPurchaseOrder) {
                    if (row.item.lots_enabled) {
                        if (!row.lot_code || !row.date_of_due)
                            errors_lots_group++;
                    }
                }
            });

            if (errors_lots_group > 0)
                return this.getCurrentResponse(
                    false,
                    "No ha registrado el lote o fecha de vencimiento para el producto"
                );
        ...
        async submit() {
            ...
            if (!validate_data_items.success)
                return this.$message.error(validate_data_items.message);
