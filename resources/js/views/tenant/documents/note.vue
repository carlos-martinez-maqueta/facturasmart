        ...
        <div class="card-body">
            <form autocomplete="off" @submit.prevent="submit">
                <div class="form-body">
                    ...
                    <div class="row" v-if="form.items.length > 0">
                        ...
                        <div class="col-md-12">
                            ...
                            <p class="text-end" v-if="form.total_isc > 0">
                                ISC: {{ currency_type.symbol }}
                                {{ form.total_isc }}
                            </p>
                            <p class="text-end" v-if="form.total_charge > 0">
                                OTROS CARGOS: {{ currency_type.symbol }}
                                {{ form.total_charge }}
                            </p>

                            <template
                                v-if="
                                    isCreditNoteAndType13 ||
                ...
                <div class="form-actions text-end mt-4">
                    ...
                    <template
                        v-if="isCreditNoteAndType13 || isCreditNoteAndType03"
                    >
                        <el-button
                            type="primary"
                            native-type="submit"
...
export default {
    ...
    async created() {
        ...
        this.form.note_credit_or_debit_type_id = "01";
    },
    mounted() {},
    computed: {
        isCreditNoteAndType13: function () {
            return (
                this.form.document_type_id === "07" &&
                this.form.note_credit_or_debit_type_id === "13"
            );
        },
        isCreditNoteAndType03: function () {
            return (
                this.form.document_type_id === "07" &&
                this.form.note_credit_or_debit_type_id === "03"
            );
        },
    ...
    methods: {
            ...
            if (this.isCreditNoteAndType13) {
                ...
                if (this.document_affected.payment_condition_id !== "02") {
                    ...
                    return this.$message.error(
                        "Para el tipo de nota de crédito seleccionada, el comprobante relacionado debe ser al crédito"
                    );
                }
        ...
        async initData() {
            ...
            if (
                this.selected_credit_note_type_13 ||
                this.selected_credit_note_type_03
            ) {
        ...
        async initFormCreditNoteAndType13() {
            ...
            this.form.total_base_other_taxes =
                this.document.total_base_other_taxes;
            this.form.total_other_taxes = this.document.total_other_taxes;
            this.form.total_plastic_bag_taxes =
                this.document.total_plastic_bag_taxes;
            ...
            this.form.hotel = {};
            this.form.charges = this.document.charges
                ? Object.values(this.document.charges)
                : null;
            this.form.payment_condition_id = null;
            this.form.fee = [];
        ...
        //remove item with total 0
        removeItemsTotalZero(items){
            //if has any item with total 0
            let hasItemTotalZero = items.some((item) => item.total == 0);
            if(hasItemTotalZero){
                this.$message.warning("Se descartaron los productos gratuitos");
            }
            return items.filter((item) => item.total > 0);
        },
        async initForm() {
            // console.log(this.hasDiscounts)
            let items = this.removeItemsTotalZero(this.document.items);
            this.errors = {};
            this.form = {
        ...
        ediItem(row, index) {
            if (
                this.form.document_type_id == "07" &&
                !this.form.note_credit_or_debit_type_id
            ) {
                return this.$message.error(
                    "Elija una opción del campo Tipo nota de crédito"
                );
            }
        ...
        getHasDocuments() {
            this.$http
                .get(
                    `/${this.resource}/note/has-documents/${this.form.affected_document_id}`
                )
                .then((response) => {
        ...
        checkPercentageIgvDebitNote() {
            this.form.items.forEach((row) => {
                if (this.applyPercentageChange(row.affectation_igv_type_id)) {
                    if (row.aux_percentage_igv == undefined)
                        row.aux_percentage_igv = row.percentage_igv;
                    row.percentage_igv = 0;
                    row.changed_percentage_debit_note = true;
                } else {
                    if (
                        row.changed_percentage_debit_note != undefined &&
                        row.changed_percentage_debit_note
                    ) {
        ...
        applyPercentageChange(affectation_igv_type_id) {
            ...
            return (
                affectations.includes(affectation_igv_type_id) &&
                this.isDebitNote &&
                debit_note_types.includes(
                    this.form.note_credit_or_debit_type_id
                )
            );
        },
