    ...
    <div>
        <div class="page-header pr-0">
            ...
            <ol class="breadcrumbs">
                <li class="active"><span>Facturas - Boletas</span></li>
            </ol>
            <div
            ...
            >
                <div class="btn-group flex-wrap" v-if="!isComercial">
                    ...
                    <button
                        v-if="!isComercial"
                        type="button"
                        @click.prevent="clickOpenMassiveNote"
                        class="btn btn-custom btn-sm mt-2 mr-2"
                    >
                        <i class="fa fa-plus-circle"></i> Notas masivas
                    </button>
                    <a
                        v-if="!isComercial"
                        :href="`/${resource}/note_other`"
                        class="btn btn-custom btn-sm mt-2 mr-2"
                        ><i class="fa fa-plus-circle"></i> Nota dcto externo</a
                    >
                    <a
                        v-if="!isComercial"
        ...
        <div class="card mb-0 w-100">
            ...
            <div class="card-body">
                ...
                >
                    ...
                    <tr slot="heading">
                        ...
                        <th v-if="columns.pending_to_delivery.visible">
                            Pte. de entrega
                        </th>
                        ...
                        <th>Estado
                            <el-tooltip
                                ...
                                >
                                    <div slot="content">
                                    <b>
                                        - Registrado: el comprobante se creó y no tiene valor legal hasta su aceptación.<br />
                                        - Enviado: el comprobante fue enviado al PSE/OSE y no tiene valor legal hasta ser aceptado.<br />
                                        - Aceptado: comprobante con valor legal. Para su conformidad, el CDR debe indicar aceptado.<br />
                                        - Observado: el comprobante tiene errores en los datos utilizados.<br />
                                        - Rechazado: documento sin valor legal por errores o falta de certificado digital activo.<br />
                                        - Anulado: documento sin valor legal por decisión del cliente.<br />
                                        - Por anular: documento probablemente sin valor legal, debe confirmar su anulación.<br />
                                        - Interno: documento no enviado a Sunat, sirve como control interno.
                                    </b>
                                    </div>
                                    <i class="fa fa-info-circle"></i>
                                </el-tooltip>
                        </th>
                        <th v-if="configuration.dispatchers_packers_document">
                            Empaquetador
                        </th>
                        <th v-if="configuration.dispatchers_packers_document">
                            Repartidor
                    ...
                    >
                        ...
                        <td v-if="columns.pending_to_delivery.visible">
                            <el-tooltip
                                v-if="row.pending_to_delivery == 2"
                                class="item"
                                effect="dark"
                                placement="bottom"
                                content="Existe mercadería pendiente de entrega"
                            >
                                <i class="fas fa-times text-danger"> </i>
                            </el-tooltip>
                            <el-tooltip
                                v-else-if="row.pending_to_delivery == 1"
                                class="item"
                                effect="dark"
                                placement="bottom"
                                content="No existe mercadería pendiente de entrega"
                            >
                        ...
                        <td v-if="configuration.dispatchers_packers_document">
                            <el-select
                                v-model="row.person_packer_id"
                                @change="
                                    changePersonPacker(
                                        row.person_packer_id,
                                        row.id
                                    )
                                "
                            ...
                            >
                                <el-option
                                    v-for="(option, idx) in person_packers"
                                    :key="idx"
                                    :value="option.id"
                                    :label="option.name"
                                ></el-option>
                        ...
                        <td v-if="configuration.dispatchers_packers_document">
                            <el-select
                                v-model="row.person_dispatcher_id"
                                @change="
                                    changePersonDispatcher(
                                        row.person_dispatcher_id,
                                        row.id
                                    )
                                "
                                style="width: 120px !important"
                            >
                                <el-option
                                    v-for="(option, idx) in person_dispatchers"
                                    :key="idx"
                                    :value="option.id"
                                    :label="option.name"
                                ></el-option>
                        ...
                        <td class="text-end">
                            ...
                            <button
                                v-if="columns.validador.visible"
                                type="button"
                                style="min-width: 41px"
                                class="btn btn-outline-primary mb-1"
                                @click.prevent="generateTxtFiles(row.id)"
                            >
                                Validar CPE
                            </button>
                            <button
            ...
            <document-for-note
                :configuration="configuration"
                :showDialog.sync="showMassiveNote"
            ></document-for-note>
        </div>
    </div>
</template>
...
import DocumentForNote from "./partials/massive_note.vue";
...
export default {
    ...
    data() {
        return {
            ...
            selectedDocumentId: null, // Para almacenar el ID del documento seleccionado
        };
    },
    created() {
        ...
        this.getUsers();
        this.getPersonPackersDispatchers();
    },
    methods: {
        clickOpenMassiveNote() {
            this.showMassiveNote = true;
        },
        async generateTxtFiles(documentId) {
            try {
                const response = await this.$http.post(
                    "/reports/validate-documents/generate-txt",
                    {
                        document_id: documentId, // Suponiendo que documentId es el ID del documento
                    }
                );
                if (response.data.success) {
                    // Maneja el éxito (puedes mostrar un mensaje o realizar alguna otra acción)
                ...
                console.error("Error generating TXT files:", error);
                this.$message.error(
                    "Ocurrió un error al generar los archivos TXT: " +
                        error.response.data.message
                );
            }
        },

        getPersonPackersDispatchers() {
            this.$http("/documents/person_packers_dispatchers").then(
                (response) => {
        ...
        changePersonPacker(person_packer_id, document_id) {
            this.$http(
                `/documents/change-person-packer/${document_id}/${person_packer_id}`
            ).then((response) => {
        ...
        changePersonDispatcher(person_dispatcher_id, document_id) {
            this.$http(
                `/documents/change-person-dispatcher/${document_id}/${person_dispatcher_id}`
            ).then((response) => {
                if (response.status == 200) {
                    this.$message.success(response.data.message);
                    this.$refs.dataTable.getRecords();
