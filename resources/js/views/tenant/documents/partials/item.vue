            ...
            <div class="form-body">
                <div class="row">
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            ></div>
                                            <el-option
                                                :label="`${ItemOptionDescriptionView(
                                                    option
                                                )}  ${getCouponName(
                                                    option.id_cupones
                                                )} `"
                            ...
                            <div class="d-flex">
                                ...
                                <div class="col-md-6">
                                    <el-checkbox
                                        ...
                                        v-model="search.order_search_stock"
                                        >Ordenar busqueda por stock
                                    </el-checkbox>
                                    <br />
                                    <el-checkbox
                                        @change="searchRemoteItems"
                                        v-model="search.order_search_price"
                                        >Ordenar busqueda por precio
                                    </el-checkbox>
                                </div>
                            </div>
                            <small
                    ...
                    <div class="col-md-4">
                        ...
                        >
                            <el-option
                                v-for="(option, idx) in itemsBrands"
                                :key="idx"
                                :label="option.name"
                                :value="option.id"
                            ></el-option>
                    ...
                    <div class="col-md-4">
                        ...
                        >
                            <el-option
                                v-for="(option, idx) in itemsCategories"
                                :key="idx"
                                :label="option.name"
                                :value="option.id"
                            ></el-option>
                    ...
                    <template v-if="form.item_unit_types.length > 0">
                        ...
                        <div class="col-md-3 col-sm-4">
                            ...
                            >
                                ...
                                <template
                                    v-if="
                                        applyChangeCurrencyItem && isFromInvoice
                                    "
                                >
                ...
                <div class="row">
                    ...
                    <div class="col-md-12 col-sm-12 mt-2">
                        <el-checkbox v-model="form.update_price"
...
<style>
.el-select-currency .el-input.el-input--small.el-input--suffix {
    width: 70px;
}
</style>
...
export default {
    ...
    methods: {
        ...
        async enabledSearchItemBySeriesDescription() {
            if (
                this.config.search_by_series_and_all &&
                this.items.length == 1
            ) {
                let item = this.items[0];
                let { lots } = item;

                let series = this.lastInput;

                let ifExists = lots.find((lot) => lot.series == series);
                console.log(
                    "🚀 ~ enabledSearchItemBySeriesDescription ~ ifExists:",
                    ifExists
                );
                ...
                if (ifExists) {
                    ...
                    this.lots = this.form.item.lots.filter((lot) => {
                        if (lot.series == series) {
                            lot.has_sale = true;
                            return true;
                        }
                        return false;
                    });
                    this.form.item.lots = this.lots;
        ...
        getUnitPriceByTotal() {
            ...
            this.form.unit_price_value = unit_price.toFixed(2);
        },
        changePrice(id) {
        ...
        removeFocus() {
            ...
            // Itera sobre los elementos y remueve la clase 'hover'
            items.forEach((item) => {
                item.classList.remove("hover");
            });
            // const selectElement = this.$refs.selectSearchNormal.$el;
        ...
        async searchRemoteItems(input) {
            console.log("🚀 ~ searchRemoteItems ~ input:", input);
            if (this.configuration.search_by_series_and_all) {
                this.removeFocus();
            }
            if (input.length > 2) {
                ...
                await this.$http
                    ...
                    .then((response) => {
                        this.items = response.data.items.map((item) => ({
                            ...item,
                            coupon_name: item.coupon_name || "No tiene cupón",
                        }));
                        this.loading_search = false;
                        this.enabledSearchItemsBarcode(input);
                        this.enabledSearchItemBySeries();
                        this.enabledSearchItemBySeriesDescription();
        ...
        async enabledSearchItemBySeries() {
            if (this.config.search_item_by_series && this.items.length == 1) {
                ...
                this.form.item.lots = this.lots;
                console.log(
                    "🚀 ~ this.lots=awaitthis.form.item.lots.map ~ this.lots:",
        ...
        async create() {
            // select-width
            ///seleciona el element con el id select-width y deshabllita el enter en él
            ...
            //     if(this.affectation_igv_types.length == 1){
            //     this.form.affectation_igv_type_id = this.affectation_igv_types[0].id;
            // }
            this.filterItems();
            if (!this.configuration.search_item_by_seres_and_all) {
        ...
        async changeItem() {
            ...
            console.log("🚀 ~ changeItem ~ this.form.item:", this.form.item);
            let { payment_conditions } = this.form.item;
            ...
            let sale_unit_price = this.form.item.sale_unit_price;

            if (
                this.configuration.condition_payment_items &&
                payment_conditions.length > 0
            ) {
                let exists = payment_conditions.find(
                    (i) => i.payment_condition_id == this.paymentConditionId
                );
                if (exists) {
                    sale_unit_price = exists.price;
                    this.$message.warning(
                        `Precio de venta actualizado por condición de pago`
                    );
                }
            }

            this.form.unit_price_value = sale_unit_price;
            ...
            let affectation_igv_type = _.find(this.affectation_igv_types, {
                id: this.form.item.sale_affectation_igv_type_id,
            });
        ...
        applyValidateStock() {
            return (
                ...
                this.isFromInvoice !== undefined &&
                this.isFromInvoice &&
                this.isUpdateDocument !== undefined &&
        ...
        async clickAddItem(key) {
            ...
            if (this.applyValidateStock()) {
                const validate_current_stock =
                    await this.validateCurrentStock();
            ...
            if (this.configuration) {
                console.log(
                    "🚀 ~ clickAddItem ~ this.configuration:",
                    this.configuration
                );
            ...
            if (this.configuration.condition_payment_items) {
            }
            ...
            this.row.payment_conditions = this.form.item.payment_conditions;
            ...
            //     has_sale: true,
            // });
            console.log("🚀 ~ clickAddItem ~ this.row.item:", this.row.item);
        ...
        async clickSelectLots() {
            if (
                this.configuration.search_by_series_and_all &&
                this.recordItem == null
            ) {
                this.lots = [];
            }
