                ...
                <div class="col-md-12 mt-2">
                    ...
                    <el-input v-model="form.customer_email">
                        <template slot="append">
                            ...
                            <!-- <el-tooltip
                                class="item"
                                content="Enviar a todos los correos electrónicos del cliente"
                                effect="dark"
                                placement="top-start"
                            >
                            <el-button
                                :loading="loading"
                                icon="el-icon-message"
                                @click.prevent="clickSendEmail(true)"
                                >Enviar a todos
                            </el-button>
...
export default {
    ...
    methods: {
        ...
        clickSendWhatsapp3() {
            ...
            formData.append(
                "message",
                "Comprobante de pago electrónico " + this.form.number
            );
            ...
            fetch(this.company.gekawa_url + "/api/create-message", {
                ...
                .then((data) => {
                    ...
                    let { message_status } = data;
                    if (message_status && message_status === "Success") {
        ...
        clickPrint(format) {
            let { split_code } = this.form;
            // let url = `/print-split/${split_code}/${format}`;
            let url = `/reports/massive-downloads/document_pdf?split_code=${split_code}&height=${format}`;
            url = encodeURI(url);
            if (split_code) {
