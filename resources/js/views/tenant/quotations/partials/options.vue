        ...
        >
            ...
            <div v-if="generate" class="row">
                ...
                <div class="col-lg-4 col-md-4">
                    <div
                        class="form-group"
                        :class="{ 'has-danger': errors.additional_information }"
                    >
                ...
                >
                    <div class="form-group">
                        <label class="control-label"
                            >Comprobante con retención</label
                        >
                        <el-switch
                ...
                <template v-if="is_document_type_invoice">
                    <!-- Crédito con cuotas -->
                    <div
                        v-show="document.payment_condition_id === '03'"
                        class="col-lg-12"
                    >
                ...
                <template v-else>
                    <template v-if="document.payments.length > 0">
                        <div class="col-lg-12">
                            <table>
                                <thead>
                                    <tr width="100%">
            ...
            <series-form
                v-if="generate && form.quotation"
                :items="form.quotation.items"
                :config="config"
            ></series-form>
            <!-- controlar series y lotes -->
            <div v-if="document.total > 0" class="col-lg-12 text-end">
                <label class="control-label">
                    OTROS CARGOS
                    <el-tooltip
                        content="Porcentaje"
                    ...
                    >
                        <el-checkbox
                            @change="chargeGlobal"
                            v-model="chargePercentage"
                        ></el-checkbox>
                    </el-tooltip>

                    <template v-if="!chargePercentage">
...
export default {
    ...
    methods: {
        ...
        clickSendWhatsapp3() {
            ...
            formData.append(
                "message",
                "Cotización generada correctamente " + this.form.identifier
            );

            const printUrl = this.getPrintUrl("ticket");
            ...
            fetch(this.company.gekawa_url+"/api/create-message", {
                method: "POST",
                body: formData,
            })
        ...
        setTotalPendingAmountRetention(amount) {
            ...
            this.document.total_pending_payment = ["02", "03"].includes(
                this.document.payment_condition_id
            )
                ? this.document.total - amount
                : 0;
            this.calculateAmountToPayments();
        },
        ...
        changePaymentCondition() {
            ...
            if (!_.isEmpty(this.document.retention)) {
                this.setTotalPendingAmountRetention(
                    this.document.retention.amount
                );
            }
        },
        clickRemoveFee(index) {
        ...
        deleteChargeGlobal() {
            let charge = _.find(this.document.charges, {
                charge_type_id: "50",
            });
            let index = this.document.charges.indexOf(charge);

            if (index > -1) {
                this.document.charges.splice(index, 1);
                this.document.total_charge = 0;
            }
        },
        chargeGlobal() {
            this.document.total = this.document.original_total;
            this.deleteChargeGlobal();
            let base = parseFloat(this.document.total);

            if (
                this.total_global_charge == 0 ||
                this.total_global_charge == ""
            ) {
                return;
            }
            let amount = 0;
            ...
            if (amount > 0 && !charge) {
                this.document.total_charge = _.round(amount, 2);
                this.document.total = _.round(
                    base + this.document.total_charge,
                    2
                );

                this.document.charges.push({
                    charge_type_id: "50",
                    description:
        ...
        getTotal() {
            if (
                !_.isEmpty(this.document.retention) &&
                this.document.total_pending_payment > 0
            ) {
        ...
        clickAddPayment() {
            ...
            if (this.document.payments === undefined)
                this.document.payments = [];
            let total = 0;
            if (
                this.form !== null &&
        ...
        assignDocument() {
            ...
            if (this.document.total_charge == 0) {
            ...
            if (
                q.additional_information !== undefined &&
                q.additional_information != null
            ) {
        ...
        async getRecord() {
            await this.$http
                ...
                .then((response) => {
                    ...
                    this.document.original_total = this.form.quotation.total;
        ...
        changePaymentMethodType(index) {
            let id = "01";
            if (
                this.document.payments[index] !== undefined &&
                this.document.payments[index].payment_method_type_id !==
                    undefined
            ) {
                id = this.document.payments[index].payment_method_type_id;
            } else if (
                this.document.fee[index] !== undefined &&
            ...
            if (payment_method_type.number_days) {
                this.document.date_of_due = moment(this.document.date_of_issue)
                ...
                let date = moment(this.document.date_of_issue)
                    .add(payment_method_type.number_days, "days")
                    .format("YYYY-MM-DD");
                ...
                if (this.document.fee !== undefined) {
                    for (
                        let index = 0;
                        index < this.document.fee.length;
                        index++
                    ) {
