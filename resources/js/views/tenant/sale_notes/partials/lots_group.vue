    ...
    >
        <div class="form-body">
            <div class="row">
                <div class="col-md-6">
                    <span
                        >Si al seleccionar lotes la cantidad es mayor, el último
                        lote quedara con la diferencia.
                    </span>
                </div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <th>
                                    <el-input-number
                                        v-model="row.compromise_quantity"
                                        v-bind:class="{
                                            'text-danger':
                                                row.compromise_quantity >
                                                row.quantity,
                                        }"
                                    ></el-input-number>
                                </th>
...
export default {
        ...
        quantityCompleted() {
            return (
                this.lots_group_
                    ...
                    .reduce(
                        (accum, item) => accum + Number(item.quantity),
                        0
                    ) >= this.quantity
            );
        },
        toAttend() {
            return (
                ...
                this.lots_group_
                    ...
                    .reduce(
                        (accum, item) =>
                            accum + Number(item.compromise_quantity),
                        0
                    )
            );
        },
    ...
    methods: {
        ...
        filter() {
            if (this.search) {
                this.lots_group_ = _.filter(this.lots_group, (x) =>
                    x.code.toUpperCase().includes(this.search.toUpperCase())
                );
            } else {
        ...
        changeSelect(event, index) {
            ...
            this.idSelected = this.lots_group_
                .filter((x) => x.checked == true)
                .map((x) => x.id);

            let sum = this.lots_group_
                .filter((x) => x.checked == true)
        ...
        async submit() {
            ...
            let compromise_quantity = this.lots_group_
                .filter((x) => x.compromise_quantity > 0)
                .reduce(
                    (accum, item) => accum + Number(item.compromise_quantity),
                    0
                );
            ...
            const successValid = this.lots_group_
                .filter((x) => x.compromise_quantity > 0)
                .filter((x) => x.compromise_quantity > x.quantity);
