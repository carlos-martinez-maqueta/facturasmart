...
if ($logo) {
    ...
    $marca_agua = public_path("storage/uploads/logos/{$company->logo}");
...
<table class="full-width">
    <tr>
        @if($company->favicon)
            <td width="20%">
                <img src="data:{{mime_content_type(public_path("{$company->favicon}"))}};base64, {{base64_encode(file_get_contents(public_path("{$company->favicon}")))}}" alt="{{$company->name}}" alt="{{ $company->name }}"  class="company_logo" style="max-width: 300px">
            </td>
        @else
            <td width="20%">
