...
<body class="ticket">
    ...
    <table class="full-width">
        ...
        @if ($document->retention)
            ...
            <tr>
                ...
                <td>
                    <p class="desc">{{ $document->currency_type->symbol }} {{ $document->retention->amount }}</p>
                </td>
            </tr>
        @endif
    ...
    <table class="full-width mt-10 mb-10">
        <thead class="">
            <tr>
                ...
                <th class="border-top-bottom desc-9 text-right">Desc.</th>
        ...
        <tbody>
            @foreach ($document->items as $row)
                <tr>
                    ...
                    <td style="font-size:11px;" class="text-right align-top">{{ number_format($row->total_discount, 2) }}</td>
                    <td style="font-size:11px;" class="text-right align-top">{{ number_format($row->total - $row->total_discount, 2) }}</td>
                </tr>
                <tr>
            ...
            @if ($document->total_exportation > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold desc">Op. Exportación:
            ...
            @if ($document->total_free > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold desc">Op. Gratuitas:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @if ($document->total_unaffected > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold desc">Op. Inafectas:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @if ($document->total_exonerated > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold desc">Op. Exoneradas:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @if ($document->document_type_id === '07')
                @if ($document->total_taxed >= 0)
                    <tr>
                        <td colspan="6" class="text-right font-bold desc">Op. Gravadas:
                            {{ $document->currency_type->symbol }}</td>
            ...
            @if ($document->total_plastic_bag_taxes > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold desc">Icbper:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @endif
            <tr>
                <td colspan="6" class="text-right font-bold desc">IGV: {{ $document->currency_type->symbol }}
                </td>
            ...
            @if ($document->total_isc > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold desc">ISC: {{ $document->currency_type->symbol }}
                    </td>
            ...
            @if ($document->total_discount > 0 && $document->subtotal > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold desc">Subtotal:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @if ($document->total_discount > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold desc">Descuento total:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @if ($document->total_charge > 0)
                @if ($document->charges)
                    ...
                    @endphp
                    <tr>
                        <td colspan="6" class="text-right font-bold desc">CARGOS ({{ $total_factor }}%):
                            {{ $document->currency_type->symbol }}</td>
                        <td class="text-right font-bold desc">{{ number_format($document->total_charge, 2) }}</td>
                    </tr>
                @else
                    <tr>
                        <td colspan="6" class="text-right font-bold desc">CARGOS:
                            {{ $document->currency_type->symbol }}</td>
                        <td class="text-right font-bold desc">{{ number_format($document->total_charge, 2) }}</td>
                    </tr>
                @endif
            ...
            <tr>
                <td colspan="6" class="text-right font-bold desc">Total a pagar:
                    {{ $document->currency_type->symbol }}</td>
            ...
            @if (($document->retention || $document->detraction) && $document->total_pending_payment > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold desc">M. Pendiente:
                        {{ $document->currency_type->symbol }}</td>
                    <td class="text-right font-bold desc">{{ number_format($document->total_pending_payment, 2) }}
                    </td>
                </tr>
            @endif
            ...
            @if ($balance < 0)
                <tr>
                    <td colspan="6" class="text-right font-bold desc">Vuelto:
                        {{ $document->currency_type->symbol }}</td>
                        ...
                        @foreach ($payments as $row)
                            <p>
                                <span class="desc">&#8226; {{ $row->payment_method_type->description }} -
                                    ...
                                    {{ $document->currency_type->symbol }} {{ $row->payment + $row->change }}</span>
                            </p>
                        @endforeach
                    @endif
                @else
    ...
    <table class="full-width">
        ...
        @endif
        <tr>
            <td class="text-center desc ">Para consultar el comprobante ingresar
                a <a href="{!! route('search.index', ['external_id' => $document->external_id]) !!}"
    ...
    @if ($document->items->where('item.id_cupones', '!=', null)->count())
        <div
            ...
            <h3 style="margin-bottom: 15px; color: #2c3e50; font-size: 16px; font-weight: bold;">Cupon</h3>
            @foreach ($document->items as $row)
                @if ($row->item->id_cupones)
                    ...
                    @if ($coupon)
                        <div
                            ...
                            @endif
                            @if ($coupon->titulo)
                                <p style="margin: 5px 0; font-size: 14px; font-weight: bold; color: #000;">
                                    {{ $coupon->titulo }}</p>
                            @endif
                            @if ($coupon->descripcion)
                                <p style="margin: 5px 0; font-size: 13px; font-weight: bold; color: #000;">
                                    {{ $coupon->descripcion }}</p>
                            @endif
                            @if ($coupon->descuento)
                                <p style="margin: 5px 0; font-size: 13px; font-weight: bold; color: #000;">
                                    <strong>Descuento:</strong> {{ $coupon->descuento }}%</p>
                            @endif
                            ...
                            @if ($coupon->barcode)
                                <p
                                    style="margin: 5px 0; font-size: 13px; font-weight: bold; color: #000; text-align: center;">
                                    <strong>Código de barras:</strong></p>
                                <div style="text-align: center;">
                                    <img src="{{ $coupon->barcode }}" alt="Barcode Image"
                                        style="max-width: 180px; height: auto; display: inline-block; margin-bottom: 10px;">
                                </div>
                            @endif
                        </div>
                    @else
                        <p style="margin: 5px 0; font-size: 13px; font-weight: bold; color: #e74c3c;">Coupon not found
                        </p>
                    @endif
                @endif
            @endforeach
