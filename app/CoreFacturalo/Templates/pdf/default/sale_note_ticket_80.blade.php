    ...
    @endforeach
        ...
        @if($document->total_taxed > 0)
            <tr>
                ...
                <td class="text-right font-bold desc">{{ number_format($document->total_taxed * 1.18, 2) }}</td>
            </tr>
        @endif
...
@if($document->items->where('item.id_cupones', '!=', null)->count())
    <div style="border: 1px solid #ccc; padding: 15px; margin-top: 20px; background-color: #f9f9f9; border-radius: 8px;">
                ...
                @if($coupon)
                    <div style="margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid #ddd;">
                        @if ($coupon->imagen)
                            ...
                            @endif
                        @if($coupon->titulo)
                            <p style="margin: 5px 0; font-size: 14px; font-weight: bold; color: #34495e;">{{ $coupon->titulo }}</p>
                        @endif
                        @if($coupon->descripcion)
                            <p style="margin: 5px 0; font-size: 13px; color: #7f8c8d;">{{ $coupon->descripcion }}</p>
                        @endif
                        @if($coupon->descuento)
                            <p style="margin: 5px 0; font-size: 13px; color: #27ae60;"><strong>Descuento:</strong> {{ $coupon->descuento }}%</p>
                        @endif
                        @if($coupon->fecha_caducidad)
                            <p style="margin: 5px 0; font-size: 13px; color: #c0392b;"><strong>Fecha de caducidad:</strong> {{ \Carbon\Carbon::parse($coupon->fecha_caducidad)->format('Y-m-d') }}</p>
                        @endif
                        @if($coupon->barcode)
                            <p style="margin: 5px 0; font-size: 13px; color: #34495e;"><strong>Código de barras:</strong> {{ $coupon->barcode }}</p>
                        @endif
                    </div>
                @else
