    ...
    <table class="full-width mt-10 mb-10">
        <thead class="">
            <tr class="bg-grey">
                ...
                <th class="border-top-bottom text-center py-2" width="10%">P.Unit</th>
                <th class="border-top-bottom text-center py-2" width="6%">Dscto</th>
                <th class="border-top-bottom text-right py-2" width="10%">P.Neto</th>
                <th class="border-top-bottom text-right py-2" width="10%">P.Venta</th>
            </tr>
        </thead>
        <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center align-top">
                        ...
                        @else
                            ...
                            {{ number_format(($row->total + $row->total_discount) / $row->quantity, 2) }}
                        @endif
                    </td>
                    ...
                    <td class="text-center align-top">
                        @if ($configurations->discounts_acc)
                            ...
                            @else
                                @if ($row->discounts)
                                    @foreach ($row->discounts as $key => $disto)
                                        <span style="">

                                            {{ round($disto->factor * 100) }}
            ...
            @if ($document->document_type_id === '07')
                @if ($document->total_taxed >= 0)
                    <tr>
                        <td colspan="6" class="text-right">Op. Gravadas: {{ $document->currency_type->symbol }}
                        </td>
            ...
            @if ($document->total_plastic_bag_taxes > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold">Icbper: {{ $document->currency_type->symbol }}
                    </td>
            ...
            @if ($document->total_charge > 0)
                ...
                @else
                    <tr>
                        <td colspan="6" class="text-right font-bold">CARGOS:
                            {{ $document->currency_type->symbol }}</td>
                        <td class="text-right font-bold">{{ number_format($document->total_charge, 2) }}</td>
                    </tr>
                @endif
            ...
            @if ($document->perception)
                <tr>
                    <td colspan="6" class="text-right font-bold">Importe total:
                        {{ $document->currency_type->symbol }}</td>
                    <td class="text-right font-bold">{{ number_format($document->total, 2) }}</td>
                </tr>
                <tr>
                    <td colspan="6" class="text-right font-bold">Percepción:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @elseif($document->retention)
                <tr>
                    <td colspan="6" class="text-right font-bold" style="font-size: 16px;">Importe total:
                        {{ $document->currency_type->symbol }}</td>
                    <td class="text-right font-bold" style="font-size: 16px;">
                        {{ number_format($document->total, 2) }}</td>
                </tr>
                <tr>
                    <td colspan="6" class="text-right">TOTAL RETENCIÓN
            ...
            @if (($document->retention || $document->detraction) && $document->total_pending_payment > 0)
                <tr>
                    <td colspan="6" class="text-right font-bold">M. Pendiente:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @if ($balance < 0)
                <tr>
                    <td colspan="6" class="text-right font-bold">Vuelto: {{ $document->currency_type->symbol }}
                    </td>
