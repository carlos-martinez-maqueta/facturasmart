...
<table class="full-width mt-5">
    ...
    <tr>
        <td class="align-top">Observation: </td>
...
<table class="full-width">
    <tr>
        <td>
            <strong>Payment conditions: {{ $document->payment_condition->name }} </strong>
        </td>
    </tr>
</table>
@endif
...
<table class="full-width">
        @foreach($document->fee as $key => $quote)
            <tr>
                <td>&#8226; {{ (empty($quote->getStringPaymentMethodType()) ? 'Cuota #'.( $key + 1) : $quote->getStringPaymentMethodType()) }} / Fecha: {{ $quote->date->format('d-m-Y') }} / Amount: {{ $quote->currency_type->symbol }}{{ $quote->amount }}</td>
            </tr>
        @endforeach
    </tr>
</table>
