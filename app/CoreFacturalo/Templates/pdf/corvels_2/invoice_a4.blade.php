...
<body>
    @if($document->state_type->id == '11'  || $document->state_type->id == '09')
        <div class="company_logo_box" style="position: absolute; text-align: center; top:30%;">
            <img src="{{ public_path('status_images' . DIRECTORY_SEPARATOR . 'anulado.png') }}" alt="anulado"
                class="" style="opacity: 0.6;">
        </div>
    @endif
        ...
        <tr>
            <td class="pl-3">
                @if ($establishment->logo ?? false)
                    <div class="company_logo_box">
                        <img src="{{ public_path($establishment->logo) }}" alt="{{ $company->name }}"
                            class="company_logo" style="max-width: 150px;">
                    </div>
                @else
                    @if ($company->logo)
                        <div class="company_logo_box">
                            <img src="{{ public_path("storage/uploads/logos/{$company->logo}") }}"
            ...
            <td >
                <div class="text-left">
                    ...
                    <h6>{{ $establishment->telephone !== '-' ? ' ' . $establishment->telephone : '' }} -
            ...
            <td width="30%" class="border-box py-4 px-3 text-center"
                style="background: url('{{ app_path('CoreFacturalo' . DIRECTORY_SEPARATOR . 'Templates' . DIRECTORY_SEPARATOR . 'pdf' . DIRECTORY_SEPARATOR . '' . $template_data->template_pdf . '' . DIRECTORY_SEPARATOR . 'bgbox.png') }}'); background-repeat: no-repeat;">
    ...
    <table class="full-widths mt-3">
        <tr>
            <td width="70%" class="pl-2 pr-4 border-box2"
                style="background: url('{{ app_path('CoreFacturalo' . DIRECTORY_SEPARATOR . 'Templates' . DIRECTORY_SEPARATOR . 'pdf' . DIRECTORY_SEPARATOR . '' . $template_data->template_pdf . '' . DIRECTORY_SEPARATOR . 'bgbox2.png') }}'); background-repeat: no-repeat;">
                <div class="border-boxs font-bold"
                    style="background: url('{{ app_path('CoreFacturalo' . DIRECTORY_SEPARATOR . 'Templates' . DIRECTORY_SEPARATOR . 'pdf' . DIRECTORY_SEPARATOR . '' . $template_data->template_pdf . '' . DIRECTORY_SEPARATOR . 'bgbox2.png') }}'); background-repeat: no-repeat;">
                    Datos del cliente
                </div>
        ...
        <td colspan="3" width="80px" class=" font-bold" style="vertical-align: top;">Datos del cliente</td>
                    ...
                    <tr>
                        <td style="vertical-align: top;" class="font-bold font-md font-xs">
                            {{ $customer->identity_document_type->description }}</td>
                    ...
                    @if ($customer->address !== '')
                        <tr>
                            ...
                            <td class="font-bold font-xs" style="vertical-align: top;">:</td>
            ...
            <td width="30%" class="border-box1 py-4 px-2 ext-center"
                style="background: url('{{ app_path('CoreFacturalo' . DIRECTORY_SEPARATOR . 'Templates' . DIRECTORY_SEPARATOR . 'pdf' . DIRECTORY_SEPARATOR . '' . $template_data->template_pdf . '' . DIRECTORY_SEPARATOR . 'bgbox1.png') }}'); background-repeat: no-repeat;">
                ...
                <table class="full-width ">
                    ...
                    <tr>
                        <td class="font-bold font-xs">Hora de emisión</td>
                        <td class="font-bold font-xs" width="8px">:</td>
                        <td class=" font-xs">{{ $document->time_of_issue }}</td>
                    </tr>



                    <tr>
                        <td class="font-bold font-xs">Moneda</td>
                        <td class="font-bold font-xs" width="8px">:</td>
                        <td class=" font-xs">{{ $document->currency_type->description }}</td>
                    </tr>
        ...
        <tbody class="colores">
            @foreach ($document->items as $row)
                <tr class="color-border">
                    ...
                    <td class="text-center align-top color-border">{{ symbol_or_code($row->item->unit_type_id) }}</td>
    ...
    <table class="full-width">
        ...
        @if ($document->total_free > 0)
            <tr>
                <td colspan="8" class="text-right font-bold">Op. Gratuitas: {{ $document->currency_type->symbol }}
                </td>
        ...
        @if ($document->total_unaffected > 0)
            <tr>
                <td colspan="8" class="text-right font-bold">Op. Inafectas: {{ $document->currency_type->symbol }}
                </td>
        ...
        @if ($document->total_exonerated > 0)
            <tr class="bg-white">
                <td colspan="8" class="text-right font-bold bg-white">Op. Exoneradas:
                    {{ $document->currency_type->symbol }}</td>
        ...
        @if ($document->total_taxed > 0)
            <tr>
                <td colspan="8" class="text-right font-bold">Op. Gravadas: {{ $document->currency_type->symbol }}
                </td>
        ...
        @if ($document->perception)
            <tr>
                <td colspan="8" class="text-right font-bold"> Importe total:
                    {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold">{{ number_format($document->total, 2) }}</td>
            </tr>
            <tr>
                <td colspan="8" class="text-right font-bold">Percepción: {{ $document->currency_type->symbol }}
                </td>
                <td class="text-right font-bold">{{ number_format($document->perception->amount, 2) }}</td>
            </tr>
            <tr>
                <td colspan="8" class="text-right font-bold">Total a pagar:
                    {{ $document->currency_type->symbol }}</td>
    ...
    <div class="border-box-number">
        @foreach (array_reverse((array) $document->legends) as $row)
            @if ($row->code == '1000')
                <span style="text-transform: uppercase;">Son: <span class="font-bold">{{ $row->value }}
                        {{ $document->currency_type->description }}</span></span>
    ...
    <div class="border-box-number-1">
        <div class="border-box-radius" style="margin-bottom: 15px;">
...
@endif --}}
        <div class="border-box-radius">
            ...
            @if ($document->detraction)
                <strong>P. Detracción </strong>:{{ $document->detraction->percentage }}%<br>
                <strong>N. Cta detracciones </strong>:{{ $document->detraction->bank_account }} <br>
                <strong>Monto detracción </strong>: S/ {{ $document->detraction->amount }}<br> <br>
            @endif
            ...
            @if ($document->payment_condition_id === '01')
                @if ($payments->count())
                    ...
                    @foreach ($payments as $row)
                        <span class="font-xs"> &#8226; {{ $row->payment_method_type->description }} -
            ...
            @else
                ...
                @foreach ($document->fee as $key => $quote)
                    <span class="font-xs"> &#8226; Cuota #{{ $key + 1 }} / Fecha:
    ...
    <div class="border-box-number-2 text-center">
    ...
    <div class="border-box-number" style="margin-top:15px;">
        @if (in_array($document->document_type->id, ['01', '03']))
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-center">Código de cuenta interbancaria</th>
                    <th class="text-center">Código de cuenta</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">{{ strtoupper($account->currency_type->description) }}</td>
                    <td class="text-center">{{ $account->cci }}</td>
