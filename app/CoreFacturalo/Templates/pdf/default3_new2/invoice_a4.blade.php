    ...
    <table class="full-width mt-0 mb-0">
        <thead>
            <tr class="">
                ...
                <th class="border-top-bottom text-center py-1 desc" class="cell-solid" width="5%">Cant.</th>
                <th class="border-top-bottom text-center py-1 desc" class="cell-solid" width="6%">U.M.</th>
                <th class="border-top-bottom text-center py-1 desc" class="cell-solid" width="35%">Descripción
                </th>

                <th class="border-top-bottom text-right py-1 desc" class="cell-solid" width="18%">Lote - F.Vcto
                </th>
                <th class="border-top-bottom text-right py-1 desc" class="cell-solid" width="12%">P.Unit</th>
                <!-- <th class="border-top-bottom text-right py-1 desc" class="cell-solid" width="12%">f-Vencimiento</th> -->
        ...
        <tbody class="">
            @foreach ($document->items as $row)
                <tr>
                    ...
                    <td class="1 text-center align-top desc cell-solid-rl">
                        {{ symbol_or_code($row->item->unit_type_id) }}</td>

                    </td>

                    <td class=" text-left align-top desc cell-solid-rl">
                        @if ($row->name_product_pdf)
                            @php
                                $name_product_pdf = str_replace(['<p>', '</p>'], '', $row->name_product_pdf);
                            @endphp
                            {{ $name_product_pdf }}
                        @else
                    ...
                    <td class="text-center desc align-top" style="white-space: nowrap;">
                    ...
                    <!-- <td class="p-1 text-center align-top desc cell-solid-rl">{{ $row->item->date_of_due ?? '' }}</td> -->


                    <td class="p-1 text-right align-top  cell-solid-rl">{{ number_format($row->unit_price, 2) }}
                    </td>
...
@endif
                    ...
                    <td class="p-1 text-right align-top  cell-solid-rl">{{ number_format($row->total, 2) }}</td>
                </tr>
            @endforeach
            ...
            <tr>
                <td class="p-1 text-left align-top desc cell-solid" colspan="3"
                    rowspan="{{ $document->retention || $document->detraction ? '7' : '6' }}">
                        ...
                        <table>
                            ...
                            <tr>
                                <td class="font-sm">Base imponible de la retención:
                                    S/
                                    {{ round($document->retention->amount_pen / $document->retention->percentage, 2) }}
                                </td>
                            </tr>
                            <tr>
                                <td class="font-sm">Porcentaje de la retención
                                    {{ $document->retention->percentage * 100 }}%</td>
                            </tr>
                            <tr>
                                <td class="font-sm">Monto de la retención S/ {{ $document->retention->amount_pen }}
                                </td>
                            </tr>
                        </table>
                    @endif
                ...
                <td class="p-1 text-center align-top desc cell-solid "
                    rowspan="{{ $document->retention || $document->detraction ? '7' : '6' }}">
            ...
            @if (($document->retention || $document->detraction) && $document->total_pending_payment > 0)
                <tr>
                    <td class="p-1 text-right align-top desc cell-solid font-bold" colspan="2">M. Pendiente:
