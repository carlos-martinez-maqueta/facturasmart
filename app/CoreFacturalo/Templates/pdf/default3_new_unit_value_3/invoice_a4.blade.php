    ...
    <table class="full-width mt-0 mb-0">
        <thead>
            <tr class="">
                ...
                <th class="border-top-bottom text-center py-1 desc" class="cell-solid" width="36%">Descripción
                </th>
                <th class="border-top-bottom text-center py-1 desc" class="cell-solid" width="6%">Modelo</th>
                <th class="border-top-bottom text-right py-1 desc" class="cell-solid" width="6%">Lote</th>
        ...
        <tbody class="">
            @foreach ($document->items as $row)
                <tr>
                    <td class="cell-solid-bottom p-1 text-center align-top desc cell-solid-rl">
                    ...
                    <td class="cell-solid-bottom p-1 text-left align-top desc text-upp cell-solid-rl">
                    ...
                    <td class="cell-solid-bottom p-1 text-right align-top desc cell-solid-rl">{{ $row->item->model ?? '' }}</td>
                    <td class="cell-solid-bottom p-1 text-center align-top desc cell-solid-rl">
                        @inject('itemLotGroup', 'App\Services\ItemLotsGroupService')
                        {{ $itemLotGroup->getLote($row->item->IdLoteSelected) }}
                    </td>
                    <td class="cell-solid-bottom p-1 text-right align-top desc cell-solid-rl">{{ number_format($row->unit_price, 2) }}
                    </td>
                    ...
                    <td class="cell-solid-bottom p-1 text-right align-top desc cell-solid-rl">{{ number_format($row->total, 2) }}</td>
                </tr>
            @endforeach
            ...
            <tr>
                <td class="p-1 text-left align-top desc cell-solid" colspan="4">
                    <table width="100%">
                        <tr>
            ...
            <tr>
                <td colspan="4" class="p-1 text-left align-top desc cell-solid" rowspan="{{ $document->retention || $document->detraction ? '7' : '6' }}">
                    <table width="100%">
                        <tr>
                            <td width="50%" style="vertical-align: top;">
                                ...
                                @if ($document->payment_method_type_id)
                                    <strong>Método de Pago: {{ $document->payment_method_type->description }}
                                    </strong><br>
                                @endif
                                ...
                                @if ($document->retention)
                                    ...
                                    <span>Base imponible: </span>{{ $document->currency_type->symbol }}
                                    {{ $document->retention->base }}<br>
                                    <span>Porcentaje: </span>{{ $document->retention->percentage * 100 }}%<br>
                                    <span>Monto: </span>{{ $document->currency_type->symbol }}
                                    {{ $document->retention->amount }}<br><br>
                                @endif
                                {{-- retencion --}}


                                @if (($document->retention || $document->detraction) && $document->total_pending_payment > 0)
                                    <strong>M. Pendiente: </strong>{{ $document->currency_type->symbol }}
