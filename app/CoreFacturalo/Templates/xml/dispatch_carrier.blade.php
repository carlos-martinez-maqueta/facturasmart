    ...
    <cac:Shipment>
        ...
        <cac:ShipmentStage>
            ...
            <!-- CONDUCTOR PRINCIPAL -->
                ...
                @if(isset($document['secondary_driver_secondary']))
                <cac:DriverPerson>
                    <!-- TIPO Y NUMERO DE DOCUMENTO DE IDENTIDAD -->
                    <cbc:ID schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06"
                            schemeAgencyName="PE:SUNAT"
                            schemeName="Documento de Identidad"
                            schemeID="{{ $document['driver_identity_document_type_id'] }}">{{ $document['secondary_driver_number'] }}</cbc:ID>
                    <!-- NOMBRES -->
                    <cbc:FirstName>{{ $document['secondary_driver_names'] }}</cbc:FirstName>
                    <!-- APELLIDOS -->
                    <cbc:FamilyName>{{ $document['secondary_driver_names'] }}</cbc:FamilyName>
                    <!-- TIPO DE CONDUCTOR: SECUNDARIO -->
                    <cbc:JobTitle>Secundario</cbc:JobTitle>
                    <cac:IdentityDocumentReference>
                        <!-- LICENCIA DE CONDUCIR SECUNDARIO -->
                        <cbc:ID>{{ $document['secondary_driver_license'] }}</cbc:ID>
                    </cac:IdentityDocumentReference>
                </cac:DriverPerson>
                @endif
        </cac:ShipmentStage>
