...
class ClientController extends Controller
{
    ...
    public function records(Request $request)
    {
        ...
        $paginatedRecords = $query->paginate(config('tenant.items_per_page'));

        $paginatedRecords->getCollection()->transform(function ($row) {
            $tenancy = app(Environment::class);
            $tenancy->tenant($row->hostname->website);
    ...
    public function startBillingCycle(Request $request)
    {
        ...
        if ($end_billing_cycle) {
            ...
            $client->save();
            //si la fecha actual es mayor o igual a la fecha de fin de ciclo de facturación
            $today = date('Y-m-d');
            if ($today >= $end_billing_cycle) {
                //crea un objeto request para simular el request de la función renewPlan
                $request_renew = new Request();
                //no reemplaces inserta el id
                $request_renew->merge([
                    'id' => $request->id,
                    'locked_tenant' => '1'
                ]);
