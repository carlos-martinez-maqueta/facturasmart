...
class CashController extends Controller
{
    ...
    public function re_open($id)
    {
        ...
        $cash->date_closed = null;
        $cash->time_closed = null;
        ...
        return [
            'success' => true,
            'message' => 'Caja re-aperturada con éxito'
        ];
    }
    ...
    public function close($id, Request $request)
    {
        ...
        $count_money = 0;
        $count_money = $request->countMoney ?? 0;
        $counter = $request->counter ?? [];
        $cash = Cash::findOrFail($id);

        // dd($cash->cash_documents);
        $cash->money_count = $count_money;
    ...
    public function getDataReport($id, $is_garage = false)
    {
        ...
        $document_item_credit = DocumentItem::whereHas('document', function ($query) use ($cash) {
            $query->where('cash_id', $cash->id)
                ->where('payment_condition_id', '02');
        })->get();
        $source = $source->merge($document_item_credit);
    ...
    public function getSaleNotesReportProducts($cash)
    {
        ...
        $sale_note_items = $sale_note_items->merge($sale_note_items_credit);
