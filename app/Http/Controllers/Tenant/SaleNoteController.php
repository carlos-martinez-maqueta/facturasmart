...
class SaleNoteController extends Controller
{
    ...
    public function columns2()
    {
        ...
        $locations = array_values(array_filter($locations, function ($location) {
            return $location['value'] != '07' && $location['value'] != '15';
        }));
        return [
    ...
    private function getRecords($request)
    {
        ...
        if($location_id){
            $type = (strlen($location_id) == 2 ? 'department' : strlen($location_id) == 4) ? 'province' : 'district';
            $records->whereHas('customer', function($query) use($location_id, $type){
                $query->where($type.'_id', $location_id);
            });
        }
        if ($request->series) {
