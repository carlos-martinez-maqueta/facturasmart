        ...
        if ($request['force_create_if_not_exist']) {
         ...
         DB::connection('tenant')->transaction(function () use ($request, $data) {
            ...
            foreach ($data['items'] as $row) {
                ...
                if(!$presentation){
                    $row_ = $row['item'];
                    if (isset($row_['cantidad_factor'])) {
                        if (key_exists('cantidad_factor', $row_)) {
                            $quantity_factor = $row_['cantidad_factor'];
                        }
    
                        if ($row_['cantidad_factor'] !== 1) {
                            $unit_type_id = Functions::valueKeyInArray($row_, 'presentation_unit_type_id');
                            if ($unit_type_id) {
                                Log::info('Se ha encontrado el tipo de unidad');
                                if (key_exists('presentation_description', $row_)) {
                                    $presentation_description = $row_['presentation_description'];
                                } else {
                                    $presentation_description = $row_['presentation_name'];
                                }
    
                                $presentation = [
                                    'description' => $presentation_description,
                                    'item_id' => $item_id,
                                    'price2' => $row_['unit_price'],
                                    'price_default' => 2,
                    ...
                    $row['item']['presentation'] = $presentation;
                }
                $sale_note_item->fill($row);
                $sale_note_item->sale_note_id = $this->sale_note->id;
                $sale_note_item->save();
