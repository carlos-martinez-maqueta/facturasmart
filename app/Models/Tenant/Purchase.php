...
class Purchase extends ModelTenant
{
    ...
    public function  getCollectionData()
    {
        ...
        $total_payments = floatval( $this->payments->sum('payment'));
        $total = floatval($total);
        $difference = round($total - $total_payments, 2); 
        $is_canceled = $difference <= 0;
        $total_canceled = $this->total_canceled;
        if ($is_canceled) {
        
            $total_canceled = true;
            Purchase::where('id', $this->id)->update(['total_canceled' => true]);
            $this->save();
        }
        /*
            alone_number
        ...
        return [
            ...
            'state_type_payment_description' => $total_canceled ? 'Pagado' : 'Pendiente de pago',
    ...
    {
        $query = $query->whereStateTypeAccepted()
            ->whereTypeUser()
        ...
        if ($params->establishment_id !== null) {
            $query = $query->where('warehouse_id', $params->establishment_id);
        }
