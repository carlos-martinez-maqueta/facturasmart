...
class SaleNoteItem extends ModelTenant
{
    ...
    public function restoreStock(){
        $item = $this->getModelItem();
        $item_id = $item->id;
        $quantity = $this->quantity;
        $warehouse_id = $this->warehouse_id;
        $itemWarehouse = ItemWarehouse::where('item_id', $item_id)->where('warehouse_id', $warehouse_id)->first();
        if ($itemWarehouse) {
            $itemWarehouse->stock += $quantity;
            $itemWarehouse->save();
        }
        $item->stock += $quantity;
        $item->save();
        if(isset($this->item->item->lots)){
            foreach($this->item->item->lots as $lot){
                $lot->has_sale = false;
                $lot->save();
            }
        }
    ...
    public function sellers()
    {
        ...
        return $this->hasOne(ItemSeller::class, 'sale_note_item_id');
    }
